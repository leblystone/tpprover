/**
 * adminManualStripeGrant
 *
 * Admin-callable function to manually write a Stripe subscription to Firestore.
 * Used when the Stripe webhook failed after a successful checkout session
 * (e.g. webhook timeout, missing price ID mapping, Stripe delivery failure).
 *
 * Mirrors the RP_PRICE_MAP in stripeWebhooks.js — keep in sync.
 */
...
// Keep in sync with stripeWebhooks.js RP_PRICE_MAP
const PLAN_MAPPING = {
  'price_1TS5C550b3cktl9XUg2Uvg5d': { key: 'researchPlusMonthly',  name: 'Research+ Monthly',  interval: 'month',    tier: 'research_plus' },
  'price_1TS5D250b3cktl9XYpr3bhT2': { key: 'researchPlusAnnual',   name: 'Research+ Annual',   interval: 'year',     tier: 'research_plus' },
  'price_1TS5DS50b3cktl9Xb3gNyL2d': { key: 'researchPlusLifetime', name: 'Research+ Lifetime', interval: 'lifetime', tier: 'research_plus' },
};
...
exports.adminManualStripeGrant = onCall(
  { cors: true },
  async (request) => {
    ...
    const { userId, priceId, reason, adminNote } = request.data;
    if (!userId) throw new HttpsError('invalid-argument', 'userId is required');

    const resolvedPriceId = priceId || 'price_1TS5D250b3cktl9XYpr3bhT2';
    const planDetails = PLAN_MAPPING[resolvedPriceId];

    if (!planDetails) {
      throw new HttpsError(
        'invalid-argument',
        `Unknown price ID: ${resolvedPriceId}. Valid IDs: ${Object.keys(PLAN_MAPPING).join(', ')}`
      );
    }
    ...
    const subscriptionData = {
      ...
      adminGrantReason: reason || 'Manual admin grant — Stripe webhook failed after checkout',
    ...
    batch.set(subRef.collection('history').doc(), {
      eventType: 'admin_manual_stripe_grant',
      title: `Admin manually granted ${planDetails.name}`,
      description: `Admin override: ${reason || 'Stripe webhook failed after checkout'}. Price: ${resolvedPriceId}. Tier: ${resolvedTier}. Granted by: ${callerEmail}.`,
      severity: 'warning',
      status: 'active',
      source: 'admin_manual',
      adminGrantedBy: callerEmail,
    ...
    logger.info(`✅ Admin manually granted Stripe subscription for user ${userId} (price: ${resolvedPriceId}, tier: ${resolvedTier}) by ${callerEmail}`);
