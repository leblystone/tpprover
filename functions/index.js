...
// Admin function to disable founder pricing
exports.adminDisableFounderPricing = onCall(
  {
    cors: true
  },
  async (request) => {
    try {
      ...
      // Update founder offer configuration
      await db.collection('appConfig').doc('founderOffer').set({
        enabled: false,
        discountPercent: 0,
        cap: 100,
        totalGranted: 0,
      ...
      logger.info('✅ Founder pricing disabled successfully');
      
      // Read back to confirm
      const doc = await db.collection('appConfig').doc('founderOffer').get();
      
      return {
        success: true,
        message: 'Founder pricing disabled',
        config: doc.data()
      };
    } catch (error) {
      logger.error('❌ Error disabling founder pricing:', error);
      throw new Error(`Failed to disable founder pricing: ${error.message}`);
    }
  }
);