...
import { Mail, Eye, Save, Send, RotateCcw, Copy, CheckCircle, HelpCircle, ChevronDown, ChevronUp, Users, Clock, Settings, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function EmailTemplateManager({ theme }) {
  ...
  const [emailTriggers, setEmailTriggers] = useState(() => {
    const saved = localStorage.getItem('tpp_email_triggers');
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background-color: ${colors.sage};">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Email Trigger Management Section */}
      <div className="p-6 rounded-lg border" style={{ borderColor: theme.border, backgroundColor: theme.cardBackground }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: theme.textLight }}>
                View and manage when each email is sent
              </p>
            </div>
          </div>
          <button
            ...
            style={{ backgroundColor: theme.secondary, color: theme.text }}
          >
            {showTriggers ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        ...
        {showTriggers && (
          <div className="space-y-4 mt-4">
            {Object.entries(emailTriggers).map(([key, trigger]) => {
              const templateName = templates[key]?.name || key;
              return (
                <div key={key} className="p-4 rounded-lg border" style={{ borderColor: theme.border, backgroundColor: theme.background }}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs ${
                          trigger.triggerType === 'event' ? 'bg-blue-100 text-blue-800' :
                          trigger.triggerType === 'scheduled' ? 'bg-purple-100 text-purple-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          {trigger.triggerType === 'event' ? 'Event' : trigger.triggerType === 'scheduled' ? 'Scheduled' : 'Manual'}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-4 text-xs" style={{ color: theme.textLight }}>
                        {trigger.triggerType === 'scheduled' && trigger.schedule && (
                      ...
                      {trigger.triggerType === 'scheduled' && trigger.dayOfWeek && (
                        <div className="mt-2 text-xs" style={{ color: theme.textLight }}>
                          <strong>Schedule:</strong> Every {trigger.dayOfWeek} at {trigger.time} ({trigger.timezone || 'EST'})
                        </div>
                      )}
                    </div>
                  </div>
