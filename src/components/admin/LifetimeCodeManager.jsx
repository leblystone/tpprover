...
import { 
  ...
  CheckCircle, XCircle, Calendar, ArrowsClockwise, Gift, Hash
} from '@phosphor-icons/react';
import {
  createLifetimeCodes,
  getLifetimeCodes,
  deleteLifetimeCode,
  deleteLifetimeCodeBatch
} from '../../services/firebase';

export default function LifetimeCodeManager({ theme }) {
  ...
  const loadCodes = async () => {
    setLoading(true);
    try {
      const fetchedCodes = await getLifetimeCodes();
  ...
  const handleGenerateCodes = async () => {
    ...
    try {
      const newCodes = await createLifetimeCodes(codeCount, batchName);
      setSuccess(`✅ Successfully generated ${newCodes.length} lifetime codes!`);
      setBatchName('');
      await loadCodes();
    } catch (err) {
  ...
  const handleExportCodes = () => {
    ...
    a.href = url;
    a.download = `lifetime-codes-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const batches = {};
  codes.forEach(code => {
    const batchKey = code.batchId || 'Unknown';
    if (!batches[batchKey]) {
      batches[batchKey] = {
        id: batchKey,
        name: code.batchName || 'Unnamed Batch',
        codes: [],
        createdAt: code.createdAt
      };
    }
    batches[batchKey].codes.push(code);
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold" style={{ color: theme.text }}>
            Lifetime Code Manager
          </h2>
          <p className="text-sm mt-1" style={{ color: theme.textLight }}>
            Generate and manage redemption codes for physical Lifetime Access Kits
          </p>
        </div>
        <button
          onClick={loadCodes}
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex-[2] min-w-[200px]">
            ...
            <input
              ...
              placeholder="e.g., Holiday 2025 Kits"
              className="w-full px-3 py-2 rounded-lg text-sm"
              style={{ 
                backgroundColor: theme.background, 
                border: `1px solid ${theme.border}`,
      ...
      >
        {loading ? (
          ...
          <div className="p-8 text-center">
            <Gift className="w-12 h-12 mx-auto mb-2" style={{ color: theme.textLight }} />
                  ...
                  <tr key={code.code} className="hover:opacity-80 transition-opacity">
                    ...
                    <td className="px-4 py-3 text-sm" style={{ color: theme.textLight }}>
                      {code.used && code.usedAt ? (
                        ...
                        new Date(code.createdAt?.toDate?.() || code.createdAt).toLocaleDateString()
                      )}
                    </td>
