...
import { useShopProducts } from '../../config/plannerProducts';

export default function CartDrawer({ open, onClose, onCheckout, loading }) {
  const { items, cartCount, cartTotal, removeItem, updateQty, clearCart, addItem } = useCart();
    ...
    items.forEach((item) => {
      ...
      if (product?.relatedProductIds?.length) {
        product.relatedProductIds.forEach((rid) => {
          if (!cartIds.has(rid)) relatedIds.add(rid);
        });
      }
    });
    ...
    <>
      ...
      >
        ...
        {suggestions.length > 0 && items.length > 0 && (
          <div className="px-6 py-3 border-t" style={{ borderColor: `${theme.text}10` }}>
            ...
            <div className="space-y-1.5">
              {suggestions.map((s) => (
                <div key={s.id} className="flex items-center gap-2 p-1.5 rounded-lg" style={{ backgroundColor: `${theme.text}04` }}>
                  {s.image && (
                    <img
                      src={typeof s.image === 'string' ? s.image : s.image?.url}
                      alt=""
