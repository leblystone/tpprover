/**
 * Admin panel route config — single source of truth for nav + routes.
 * Drives both React Router paths and AdminLayout primary/secondary nav.
 */

export const ADMIN_BASE = '/admin';

/** Primary tabs (top-level nav). Each can have optional `children` for secondary nav. */
export const adminPrimaryTabs = [
  {
    id: 'overview',
    label: 'Overview',
    path: `${ADMIN_BASE}/overview/dashboard`,
    icon: 'Chalkboard',
    children: [
      { id: 'dashboard', label: 'Dashboard', path: `${ADMIN_BASE}/overview/dashboard`, icon: 'Chalkboard' },
      { id: 'analytics', label: 'Analytics', path: `${ADMIN_BASE}/overview/analytics`, icon: 'ChartLine' },
      { id: 'revenue', label: 'Revenue', path: `${ADMIN_BASE}/overview/revenue`, icon: 'CurrencyDollar' },
      { id: 'ghosty', label: 'Ghosty', path: `${ADMIN_BASE}/overview/automation`, icon: 'Robot' },
      { id: 'sync-errors', label: 'Sync Errors', path: `${ADMIN_BASE}/overview/sync-errors`, icon: 'WarningCircle' },
      { id: 'contact', label: 'Contact', path: `${ADMIN_BASE}/overview/contact`, icon: 'Envelope' },
    ],
  },
  {
    id: 'users',
    label: 'Users',
    path: `${ADMIN_BASE}/users/subscriptions`,
    icon: 'Users',
    children: [
      { id: 'subscriptions', label: 'All Users', path: `${ADMIN_BASE}/users/subscriptions`, icon: 'Users' },
      { id: 'lifetime', label: 'Lifetime', path: `${ADMIN_BASE}/users/lifetime`, icon: 'Medal' },
      { id: 'annual', label: 'Annual', path: `${ADMIN_BASE}/users/annual`, icon: 'Calendar' },
      { id: 'gifts', label: 'Gifts', path: `${ADMIN_BASE}/users/gifts`, icon: 'Gift', disabled: true },
      { id: 'expired-trials', label: 'Expired Trials', path: `${ADMIN_BASE}/users/expired-trials`, icon: 'Hourglass' },
    ],
  },
  {
    id: 'shop',
    label: 'Shop',
    path: `${ADMIN_BASE}/shop/products`,
    icon: 'Storefront',
    children: [
      { id: 'products', label: 'Products', path: `${ADMIN_BASE}/shop/products`, icon: 'Package' },
      { id: 'analytics', label: 'Analytics', path: `${ADMIN_BASE}/shop/analytics`, icon: 'ChartBar' },
      { id: 'reviews', label: 'Reviews', path: `${ADMIN_BASE}/shop/reviews`, icon: 'Star' },
      { id: 'orders', label: 'Orders', path: `${ADMIN_BASE}/shop/orders`, icon: 'ShoppingCart' },
      { id: 'inquiries', label: 'Inquiries', path: `${ADMIN_BASE}/shop/inquiries`, icon: 'ChatCircle' },
      { id: 'marketplaces', label: 'Marketplaces', path: `${ADMIN_BASE}/shop/marketplaces`, icon: 'Storefront' },
    ],
  },
  {
    id: 'comms',
    label: 'Communications',
    path: `${ADMIN_BASE}/comms/emails`,
    icon: 'MailOpen',
    children: [
      { id: 'announcements', label: 'Announcements', path: `${ADMIN_BASE}/comms/announcements`, icon: 'Megaphone' },
      { id: 'emails', label: 'Email Templates', path: `${ADMIN_BASE}/comms/emails`, icon: 'Envelope' },
      { id: 'triggers', label: 'Email Triggers', path: `${ADMIN_BASE}/comms/triggers`, icon: 'Lightning' },
      { id: 'history', label: 'Send History', path: `${ADMIN_BASE}/comms/history`, icon: 'ClockCounterClockwise' },
      { id: 'notifications', label: 'Push Notifications', path: `${ADMIN_BASE}/comms/notifications`, icon: 'Bell' },
    ],
  },
  {
    id: 'settings',
    label: 'Settings',
    path: `${ADMIN_BASE}/settings/security`,
    icon: 'GearSix',
    children: [
      { id: 'security', label: 'Security', path: `${ADMIN_BASE}/settings/security`, icon: 'Shield' },
      { id: 'deletions', label: 'Deletions', path: `${ADMIN_BASE}/settings/deletions`, icon: 'Trash' },
      { id: 'version', label: 'Version', path: `${ADMIN_BASE}/settings/version`, icon: 'DeviceMobile' },
      { id: 'agreements', label: 'Legal', path: `${ADMIN_BASE}/settings/agreements`, icon: 'Scales' },
      { id: 'flags', label: 'Kill Switches', path: `${ADMIN_BASE}/settings/flags`, icon: 'Lightning' },
      { id: 'ai-costs', label: 'AI Costs', path: `${ADMIN_BASE}/settings/ai-costs`, icon: 'Robot' },
      { id: 'content', label: 'Content', path: `${ADMIN_BASE}/content`, icon: 'BookOpen' },
      { id: 'improvements', label: 'Ideas', path: `${ADMIN_BASE}/improvements`, icon: 'Lightbulb' },
    ],
  },
];

/** Default redirect when visiting /admin */
export const adminDefaultPath = `${ADMIN_BASE}/overview/dashboard`;

/** All leaf paths (for redirects, etc.) */
export const adminPaths = {
  // Overview paths
  overviewDashboard: `${ADMIN_BASE}/overview/dashboard`,
  overviewSupport: `${ADMIN_BASE}/overview/dashboard`, // redirect: support removed, use dashboard
  overviewContact: `${ADMIN_BASE}/overview/contact`,
  overviewAnalytics: `${ADMIN_BASE}/overview/analytics`,
  overviewRevenue: `${ADMIN_BASE}/overview/revenue`,
  overviewGhosty: `${ADMIN_BASE}/overview/automation`,
  overviewAutomation: `${ADMIN_BASE}/overview/automation`,
  overviewSyncErrors: `${ADMIN_BASE}/overview/sync-errors`,
  // Legacy paths for backward compatibility
  analytics: `${ADMIN_BASE}/overview/analytics`,
  ghostWorker: `${ADMIN_BASE}/overview/automation`,
  workQueue: `${ADMIN_BASE}/overview/dashboard`,
  feedback: `${ADMIN_BASE}/overview/dashboard`,
  contact: `${ADMIN_BASE}/overview/contact`,
  // Users paths
  usersSubscriptions: `${ADMIN_BASE}/users/subscriptions`,
  usersLifetime: `${ADMIN_BASE}/users/lifetime`,
  usersAnnual: `${ADMIN_BASE}/users/annual`,
  usersGifts: `${ADMIN_BASE}/users/gifts`,
  usersExpiredTrials: `${ADMIN_BASE}/users/expired-trials`,
  // Content paths
  content: `${ADMIN_BASE}/content`,
  improvements: `${ADMIN_BASE}/improvements`,
  // Comms paths
  commsEmails: `${ADMIN_BASE}/comms/emails`,
  commsTriggers: `${ADMIN_BASE}/comms/triggers`,
  commsHistory: `${ADMIN_BASE}/comms/history`,
  commsAnnouncements: `${ADMIN_BASE}/comms/announcements`,
  commsNotifications: `${ADMIN_BASE}/comms/notifications`,
  // Legacy comms paths
  commsPush: `${ADMIN_BASE}/comms/notifications`,
  commsInApp: `${ADMIN_BASE}/comms/announcements`,
  // Settings paths
  settingsSecurity: `${ADMIN_BASE}/settings/security`,
  settingsDeletions: `${ADMIN_BASE}/settings/deletions`,
  settingsVersion: `${ADMIN_BASE}/settings/version`,
  settingsAgreements: `${ADMIN_BASE}/settings/agreements`,
  settingsFlags: `${ADMIN_BASE}/settings/flags`,
  settingsAiCosts: `${ADMIN_BASE}/settings/ai-costs`,
};
