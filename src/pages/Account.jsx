  ...
  export default function Account() {
    ...
    return (
      <section className="space-y-6">
        ...
        <div className="rounded-lg border p-6 content-card shadow-sm" style={{ borderColor: theme.border, backgroundColor: theme.cardBackground }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold" style={{ color: theme.primaryDark }}>Subscription</h2>
            {sub && sub.status !== 'trialing' && (
              <div className="flex items-center gap-2">
                <button className="px-3 py-2 rounded-md text-sm font-semibold" style={{ backgroundColor: theme.white, border: `1px solid ${theme.border}`, color: theme.text }} onClick={() => setManageOpen(true)}>Manage</button>
                {sub.interval !== 'lifetime' && (
                  <button className="px-3 py-2 rounded-md text-sm font-semibold" style={{ backgroundColor: theme.primary, color: theme.textOnPrimary }} onClick={() => setManageOpen(true)}>Upgrade</button>
                )}
              </div>
            )}
          </div>
          {!sub ? (
            <div className="text-center p-6 rounded-lg" style={{ backgroundColor: theme.background }}>
              <div className="font-semibold text-lg mb-2">Beta Access Countdown</div>
              <p className="text-sm text-gray-600 mb-4">Your free beta access ends on September 6th. Please select a plan to continue service after the beta period.</p>
              {timeLeft && (timeLeft.days > 0 || timeLeft.hours > 0 || timeLeft.minutes > 0 || timeLeft.seconds > 0) ? (
                <div className="flex justify-center items-center gap-4 text-2xl font-mono my-4 p-4 rounded-lg" style={{ backgroundColor: theme.secondary }}>
                ...
                <div className="text-lg font-semibold my-4 p-4 rounded-lg" style={{ backgroundColor: theme.secondary }}>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div className="rounded-lg border p-4" style={{ borderColor: theme.border }}>
                  ...
                  <div className="font-semibold">{sub.plan}</div>
                  <div className="mt-1 text-xs">Status: <span className="font-medium capitalize">{sub.status}</span></div>
                </div>
                <div className="rounded-lg border p-4" style={{ borderColor: theme.border }}>
                  <div className="text-xs flex items-center gap-1" style={{ color: theme.textLight }}><Calendar className="h-3 w-3" /> {sub.status === 'trialing' ? 'Trial ends' : 'Renews on'}</div>
                  <div className="font-semibold">{new Date(sub.currentPeriodEnd).toLocaleDateString()}</div>
                </div>
                <div className="rounded-lg border p-4" style={{ borderColor: theme.border }}>
                  <div className="text-xs flex items-center gap-1" style={{ color: theme.textLight }}><CreditCard className="h-3 w-3" /> Payment Method</div>
                  <div className="font-semibold">{sub.paymentMethod ? `${sub.paymentMethod.brand} •••• ${sub.paymentMethod.last4}` : '—'}</div>
                </div>
              </div>
              
              {sub.status === 'trialing' && new Date(sub.currentPeriodEnd) < new Date() && (
                <div className="p-4 rounded-lg text-center" style={{ backgroundColor: theme.accent, color: theme.accentText }}>
                  <div className="font-semibold text-lg">Your trial has ended!</div>
                  <p className="text-sm mt-1">Please choose a plan to continue enjoying full access.</p>
                </div>
              )}

              {/* {(sub.status === 'trialing' || sub.status === 'canceled') && (
                <div className="mt-6">
                  <div className="text-center font-semibold text-lg mb-4" style={{ color: theme.primaryDark }}>Choose Your Plan</div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="rounded-lg border p-6 text-center hover:shadow-lg transition-shadow" style={{ borderColor: theme.border }}>
                      <div className="font-bold text-lg">Monthly</div>
                      <div className="text-3xl font-bold my-3">$9.99<span className="text-base font-normal text-gray-500">/mo</span></div>
                      <ul className="text-sm space-y-2 my-4 text-gray-600">
                        <li>Track all your peptides</li>
                        <li>Unlimited orders & vendors</li>
                        <li>Advanced analytics</li>
                      </ul>
                      <button className="w-full px-4 py-2 rounded-md text-sm font-semibold hover:opacity-90" style={{ backgroundColor: theme.primary, color: theme.white }} onClick={() => createSubscription({ name: 'Pro Monthly', price: 9.99, interval: 'month' })}>Choose Monthly</button>
                    </div>
                    <div className="relative rounded-lg border-2 p-6 text-center hover:shadow-lg transition-shadow" style={{ borderColor: theme.primary }}>
                      <div className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 px-3 py-1 text-xs font-semibold rounded-full" style={{ backgroundColor: theme.primary, color: theme.white }}>Most Popular</div>
                      <div className="font-bold text-lg">Annual</div>
                      <div className="text-3xl font-bold my-3">$79.99<span className="text-base font-normal text-gray-500">/yr</span></div>
                      <ul className="text-sm space-y-2 my-4 text-gray-600">
                        <li><b>Save 33%</b> vs monthly</li>
                        <li>All Pro features included</li>
                        <li>Priority support</li>
                      </ul>
                      <button className="w-full px-4 py-2 rounded-md text-sm font-semibold hover:opacity-90" style={{ backgroundColor: theme.primary, color: theme.white }} onClick={() => createSubscription({ name: 'Pro Annual', price: 79.99, interval: 'year' })}>Choose Annual</button>
                    </div>
                    <div className="rounded-lg border p-6 text-center hover:shadow-lg transition-shadow" style={{ borderColor: theme.border }}>
                      <div className="font-bold text-lg">Lifetime</div>
                      <div className="text-3xl font-bold my-3">$149<span className="text-base font-normal text-gray-500"> one-time</span></div>
                      <ul className="text-sm space-y-2 my-4 text-gray-600">
                        <li>Pay once, use forever</li>
                        <li>All future updates included</li>
                        <li>The ultimate value</li>
                      </ul>
                      <button className="w-full px-4 py-2 rounded-md text-sm font-semibold hover:opacity-90" style={{ backgroundColor: theme.primary, color: theme.white }} onClick={() => createSubscription({ name: 'Lifetime', price: 149.00, interval: 'lifetime' })}>Choose Lifetime</button>
                    </div>
                  </div>
                </div>
              )} */}
        ...
        <div className="rounded-lg border p-6 content-card shadow-sm" style={{ borderColor: theme.border, backgroundColor: theme.cardBackground }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button className="px-3 py-2 rounded-md text-sm" style={{ backgroundColor: theme.cardBackground, border: `1px solid ${theme.border}` }} onClick={() => setBilling(loadBilling())} title="Refresh"><RefreshCw className="h-4 w-4" /></button>
              <button className="px-3 py-2 rounded-md text-sm font-semibold hover:opacity-90" style={{ backgroundColor: theme.primary, color: theme.textOnPrimary }} onClick={addTestInvoice}>Add Test Invoice</button>
            </div>
          </div>
          {billing.length === 0 ? (
            <div className="text-sm" style={{ color: theme.textLight }}>No invoices yet.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="text-xs uppercase" style={{ color: theme.textLight }}>
                ...
                <tbody>
                  {billing.map(inv => (
                    <tr key={inv.id} className="border-t" style={{ borderColor: theme.border }}>
                      <td className="py-3 pr-3">{new Date(inv.date).toLocaleDateString()}</td>
                      <td className="py-3 pr-3">{inv.description || 'Charge'}</td>
                      <td className="py-3 pr-3">${(Number(inv.amount)||0).toFixed(2)} {inv.currency || 'USD'}</td>
                      <td className="py-3 pr-3">{inv.status}</td>
                      <td className="py-3 pr-3 text-right">
                        <button className="p-1 rounded hover:bg-gray-100" title="Remove" onClick={() => {
                          const next = billing.filter(b => b.id !== inv.id)
                          saveBilling(next)
                          setBilling(next)
